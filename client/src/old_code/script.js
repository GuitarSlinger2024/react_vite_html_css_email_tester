...
document.querySelector('.header-nav-btns.left').onclick = e => {
  ...
  document
    .querySelectorAll('.header-nav-btns.left li')
...
function fetchCode(action) {
  ...
  //   const checkedBtn = radioBtns.querySelector(
  //     'aj-checkbox[data-checked="true"]'
  //   )
...
zipFileInput.onchange = () => {
  ...
  const folder = checkBoxes.querySelector('aj-checkbox[data-checked="true"]')
    .dataset.value
  console.log(dropMsg)
  ...
  fetch('includes/_upload_form.php', {
    ...
    .then(data => {
      ...
      if (data['error'] && data['error'] === 'file_exists') {
        utils.alertMessage(
          [
            '<div>A file already exits named:</div><div class="ellipsContainer"><span class="ellipsis">' +
              data['file'] +
              '</span></div>',
          ],
          true,
        ...
        utils.alertMessage(
          [
            '<div class= "ellipsContainer"><span class="ellipsis">' +
              data['file'] +
              '</span></div>was successfully uploaded<br><br>',
          ],
          true,
          dropMsg
        )
